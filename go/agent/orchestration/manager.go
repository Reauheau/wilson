...
func (m *ManagerAgent) SetRegistry(registry *agent.Registry) {
	m.registry = registry
}
...
func (m *ManagerAgent) ExecuteTaskPlan(ctx context.Context, parentTaskID int) error {
	...
	for _, task := range subtasks {
		...
		agentInstance := m.getAgentForTaskType(task.Type)
		if agentInstance == nil {
			return fmt.Errorf("no agent found for task type: %s", task.Type)
		}

		ui.Printf("[ManagerAgent] Task %s (type=%s) → %s agent\n", task.TaskKey, task.Type, agentInstance.Name())

		// Assign task
		if err := m.AssignTaskToAgent(ctx, task.ID, agentInstance.Name()); err != nil {
			return fmt.Errorf("failed to assign task %s: %w", task.TaskKey, err)
		}

		// Start task (marks as in_progress)
		if err := m.StartTask(ctx, task.ID, agentInstance.Name()); err != nil {
		...
		// Check if agent supports ExecuteWithContext (specialized agents like CodeAgent)
		type contextExecutor interface {
			ExecuteWithContext(context.Context, *base.TaskContext) (*agent.Result, error)
		}

		var result *agent.Result
		var err error
		if execWithCtx, ok := agentInstance.(contextExecutor); ok {
			result, err = execWithCtx.ExecuteWithContext(ctx, taskCtx)
		} else {
			// Fallback: convert TaskContext to Task and use Execute
			basicTask := &agent.Task{
			...
			result, err = agentInstance.Execute(ctx, basicTask)
		}
...
func (m *ManagerAgent) getAgentForTaskType(taskType ManagedTaskType) agent.Agent {
	if m.registry == nil {
		return nil
	}
...
func (m *ManagerAgent) extractArtifactIDs(result *agent.Result) []int {
	var ids []int
...
func (m *ManagerAgent) HandleUserRequest(ctx context.Context, userRequest string) (*agent.Result, error) {
...
func (m *ManagerAgent) handleComplexRequest(ctx context.Context, request string) (*agent.Result, error) {
...
func (m *ManagerAgent) handleSimpleRequest(ctx context.Context, request string) (*agent.Result, error) {
	// Determine task type from request
	taskType := m.inferTaskType(request)

	// Get appropriate agent
	agentInstance := m.getAgentForTaskType(taskType)
	if agentInstance == nil {
...
func (m *ManagerAgent) loadRequiredFiles(taskCtx *base.TaskContext) error {
...
func (m *ManagerAgent) injectDependencyContext(task *ManagedTask, taskCtx *base.TaskContext) error {
	...
	if len(taskCtx.DependencyFiles) > 0 {
		...
		// ✅ CRITICAL: Inject into BOTH task.Input AND taskCtx.Input
		// TaskContext.Input is what gets converted back to Task.Input in ConvertTaskContextToTask()
		// So we MUST update taskCtx.Input, not just task.Input
		if taskCtx.Input == nil {
			taskCtx.Input = make(map[string]interface{})
		}
		taskCtx.Input["dependency_files"] = taskCtx.DependencyFiles
		fmt.Printf("[ManagerAgent] Added dependency_files to taskCtx.Input for prompt visibility\n")
	}
...
func (m *ManagerAgent) StartFeedbackProcessing(ctx context.Context) {
	...
	// Register handlers
	bus.RegisterHandler(agent.FeedbackTypeDependencyNeeded, m.handleDependencyRequest)
	bus.RegisterHandler(agent.FeedbackTypeRetryRequest, m.handleRetryRequest)
...
func (m *ManagerAgent) handleDependencyRequest(ctx context.Context, fb *agent.AgentFeedback) error {
	fmt.Printf("[ManagerAgent] Processing dependency request from %s for task %s\n",
		fb.AgentName, fb.TaskID)

	// Cast TaskContext from interface{} to *base.TaskContext
	var taskCtx *base.TaskContext
	if fb.TaskContext != nil {
		var ok bool
		taskCtx, ok = fb.TaskContext.(*base.TaskContext)
		if !ok {
			fmt.Printf("[ManagerAgent] Warning: TaskContext is not of type *base.TaskContext\n")
		}
	}
	...
	if taskCtx != nil {
		...
		if fixMode, ok := taskCtx.Input["fix_mode"].(bool); ok && fixMode {
			fmt.Printf("[ManagerAgent] Task %s is already a fix task - escalating to prevent recursion\n",
				fb.TaskID)
			return m.escalateToUser(ctx, fb)
		}
	...
	depDesc, ok := fb.Context["dependency_description"].(string)
	if !ok {
		return fmt.Errorf("missing dependency_description")
	}

	depTypeStr, ok := fb.Context["dependency_type"].(string)
	if !ok {
	...
	if taskCtx != nil && taskCtx.TaskKey != "" {
		// Use TaskKey from TaskContext (e.g., "TASK-001")
		fmt.Sscanf(taskCtx.TaskKey, "TASK-%d", &currentTaskID)
	} else {
		// Fallback: try to parse TaskID directly
		fmt.Sscanf(fb.TaskID, "TASK-%d", &currentTaskID)
	}
	...
	if taskCtx != nil {
		if len(taskCtx.DependencyFiles) > 0 {
			depTask.Input["dependency_files"] = taskCtx.DependencyFiles
			fmt.Printf("[ManagerAgent] Preserved %d dependency files for fix task\n",
				len(taskCtx.DependencyFiles))
		}

		// ✅ ADD ERROR CONTEXT for dependency to learn from
		if lastErr := taskCtx.GetLastError(); lastErr != nil {
	...
	if errorType, ok := fb.Context["error_type"].(string); ok {
		if strings.Contains(errorType, "error") || strings.Contains(errorType, "compile") {
			...
			depTask.Input["compile_error"] = fb.Context["error_message"]
			depTask.Input["error_type"] = errorType
			depTask.Input["error_analysis"] = fb.Context
			...
			targetPath := fb.Context["target_path"].(string)
			...
			if tf, ok := fb.Context["target_file"].(string); ok && tf != "" {
				targetFile = tf
				fmt.Printf("[ManagerAgent] Using target file from feedback: %s\n", targetFile)
			} else {
				// Fallback: try to extract from error message
				if errorMsg, ok := fb.Context["error_message"].(string); ok {
	...
	agentInstance := m.getAgentForTaskType(depTask.Type)
	if agentInstance == nil {
		return fmt.Errorf("no agent found for fix task type: %s", depTask.Type)
	}

	// Assign fix task
	if err := m.AssignTaskToAgent(ctx, depTask.ID, agentInstance.Name()); err != nil {
		return fmt.Errorf("failed to assign fix task: %w", err)
	}

	// Start fix task
	if err := m.StartTask(ctx, depTask.ID, agentInstance.Name()); err != nil {
	...
	var fixResult *agent.Result
	var fixErr error
	if execWithCtx, ok := agentInstance.(contextExecutor); ok {
		fixResult, fixErr = execWithCtx.ExecuteWithContext(ctx, fixTaskCtx)
	} else {
		// Fallback: convert to Task
		basicTask := &agent.Task{
			ID:          fmt.Sprintf("%d", depTask.ID),
			Type:        string(depTask.Type),
			Description: depTask.Description,
			Input:       depTask.Input,
			Priority:    depTask.Priority,
			Status:      agent.TaskInProgress,
		}
		fixResult, fixErr = agentInstance.Execute(ctx, basicTask)
	}

	if fixErr != nil {
		// Fix task failed - block it and escalate
		m.BlockTask(ctx, depTask.ID, fixErr.Error())
		return fmt.Errorf("fix task %s failed: %w", depTask.TaskKey, fixErr)
	}
...
func (m *ManagerAgent) handleRetryRequest(ctx context.Context, fb *agent.AgentFeedback) error {
	// Cast TaskContext from interface{} to *base.TaskContext
	var taskCtx *base.TaskContext
	if fb.TaskContext != nil {
		var ok bool
	...
	var taskID int
	fmt.Sscanf(fb.TaskID, "TASK-%d", &taskID)
	if taskID > 0 {
...
func (m *ManagerAgent) escalateToUser(ctx context.Context, fb *agent.AgentFeedback) error {
	...
	// Cast TaskContext from interface{} to *base.TaskContext
	if fb.TaskContext != nil {
		if taskCtx, ok := fb.TaskContext.(*base.TaskContext); ok {
