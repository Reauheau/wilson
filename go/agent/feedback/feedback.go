...
type FeedbackBus struct {
	feedbackChan chan *agent.AgentFeedback
	mu           sync.RWMutex
	handlers     map[agent.FeedbackType]FeedbackHandler
	db           *sql.DB // For persistence (optional)
}

// FeedbackHandler processes specific feedback types
type FeedbackHandler func(context.Context, *agent.AgentFeedback) error
...
func GetFeedbackBus() *FeedbackBus {
	globalFeedbackBusOnce.Do(func() {
		globalFeedbackBus = &FeedbackBus{
			feedbackChan: make(chan *agent.AgentFeedback, 100), // Buffered
...
func (fb *FeedbackBus) Send(feedback *agent.AgentFeedback) error {
...
func (fb *FeedbackBus) SendAndWait(ctx context.Context, feedback *agent.AgentFeedback) error {
	feedback.CreatedAt = time.Now()
...
func (fb *FeedbackBus) RegisterHandler(feedbackType agent.FeedbackType, handler FeedbackHandler) {
	fb.mu.Lock()
	defer fb.mu.Unlock()
...
func (fb *FeedbackBus) processFeedback(ctx context.Context, feedback *agent.AgentFeedback) {
...
func (fb *FeedbackBus) persistFeedback(feedback *agent.AgentFeedback) error {
	if fb.db == nil {
...
func (fb *FeedbackBus) updateFeedbackProcessed(feedback *agent.AgentFeedback, handlerErr error) {
	if fb.db == nil {
		return
	}
...
func (fb *FeedbackBus) GetFeedbackForTask(taskID string) ([]*agent.AgentFeedback, error) {
	if fb.db == nil {
	...
	var feedbacks []*agent.AgentFeedback
	for rows.Next() {
		var (
			id            int
			taskID        string
		...
		feedback := &agent.AgentFeedback{
			TaskID:       taskID,
			AgentName:    agentName,
			FeedbackType: agent.FeedbackType(feedbackType),
			Severity:     agent.FeedbackSeverity(severity),
...
func (fb *FeedbackBus) GetRecentFeedback(limit int) ([]*agent.AgentFeedback, error) {
	if fb.db == nil {
		return nil, fmt.Errorf("database not configured")
	}
	...
	for rows.Next() {
		...
		feedbacks = append(feedbacks, &agent.AgentFeedback{
			TaskID:       taskID,
			AgentName:    agentName,
			FeedbackType: agent.FeedbackType(feedbackType),
			Severity:     agent.FeedbackSeverity(severity),
