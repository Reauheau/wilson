...
func NewReviewAgent(llmManager *llm.Manager, contextMgr *contextpkg.Manager) *ReviewAgent {
	base := base.NewBaseAgent("Review", llm.PurposeAnalysis, llmManager, contextMgr) // Use analysis model
...
func (a *ReviewAgent) CanHandle(task *agent.Task) bool {
	return task.Type == "review"
}

// ExecuteWithContext executes a task with full TaskContext
func (a *ReviewAgent) ExecuteWithContext(ctx context.Context, taskCtx *base.TaskContext) (*agent.Result, error) {
...
func (a *ReviewAgent) Execute(ctx context.Context, task *agent.Task) (*agent.Result, error) {
	result := &agent.Result{
		TaskID: task.ID,
		Agent:  a.Name(),
	}
...
func (a *ReviewAgent) checkPreconditions(ctx context.Context, task *agent.Task) error {
	// Check: Has code to review in dependency_files
	if depFiles, ok := task.Input["dependency_files"].([]string); !ok || len(depFiles) == 0 {
		return a.RequestDependency(ctx,
			"Create code to review",
			string(agent.TaskTypeCode),
...
func (a *ReviewAgent) buildUserPrompt(task *agent.Task, currentCtx *contextpkg.Context) string {
