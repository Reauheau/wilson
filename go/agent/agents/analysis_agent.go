...
func NewAnalysisAgent(llmManager *llm.Manager, contextMgr *contextpkg.Manager) *AnalysisAgent {
	base := base.NewBaseAgent("analysis", llm.PurposeAnalysis, llmManager, contextMgr)
...
func (a *AnalysisAgent) CanHandle(task *agent.Task) bool {
	return task.Type == agent.TaskTypeResearch ||
		task.Type == agent.TaskTypeAnalysis ||
		task.Type == agent.TaskTypeSummary
}

// ExecuteWithContext executes a task with full TaskContext
func (a *AnalysisAgent) ExecuteWithContext(ctx context.Context, taskCtx *base.TaskContext) (*agent.Result, error) {
...
func (a *AnalysisAgent) Execute(ctx context.Context, task *agent.Task) (*agent.Result, error) {
	result := &agent.Result{
		TaskID: task.ID,
		Agent:  a.Name(),
	}
	...
	if task.Type == agent.TaskTypeResearch {
		artifactType = "research_result"
	} else if task.Type == agent.TaskTypeSummary {
...
func (a *AnalysisAgent) buildSystemPrompt(task *agent.Task) string {
...
func (a *AnalysisAgent) buildUserPrompt(task *agent.Task, currentCtx *contextpkg.Context) string {
