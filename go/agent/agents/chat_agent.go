...
func NewChatAgent(llmManager *llm.Manager, contextMgr *contextpkg.Manager) *ChatAgent {
	baseAgent := base.NewBaseAgent("chat", llm.PurposeChat, llmManager, contextMgr)
...
func (a *ChatAgent) CanHandle(task *agent.Task) bool {
	// Chat agent can handle any general task
	return task.Type == agent.TaskTypeGeneral || task.Type == ""
}

// ExecuteWithContext executes a task with full TaskContext
func (a *ChatAgent) ExecuteWithContext(ctx context.Context, taskCtx *base.TaskContext) (*agent.Result, error) {
...
func (a *ChatAgent) Execute(ctx context.Context, task *agent.Task) (*agent.Result, error) {
	result := &agent.Result{
		TaskID: task.ID,
		Agent:  a.Name(),
	}
...
func (a *ChatAgent) buildUserPrompt(task *agent.Task, currentCtx *contextpkg.Context) string {
